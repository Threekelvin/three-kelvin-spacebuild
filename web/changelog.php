...
$top =	  "<html><head><style type='text/css'> html, body { background:#979797; } "
		. ".entry { background:#c9c9c9; border-radius:5px; margin:5px; padding:10px; overflow:hidden; min-height:64px; font-family:Tahoma,Arial,sans-serif; } "
		. ".avatar { float:right; } img { margin:0; } ul { margin:0; } "
...
foreach($commits as $date => $users) {
	...
	foreach($users as $user => $messages) {
		$middle .= "<div><u>" . $user . "</u><div class='avatar'><img src='" . getAvatarUri($user) . "' width='64' height='64'/></div><br><ul>";
