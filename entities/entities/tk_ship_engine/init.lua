...
local function math_physvec(vec)
    return Vector(vec.x > -math.huge and (vec.x < math.huge and vec.x or 0) or 0, vec.y > -math.huge and (vec.y < math.huge and vec.y or 0) or 0, vec.z > -math.huge and (vec.z < math.huge and vec.z or 0) or 0)
end

local function math_qmul(lhs, rhs)
    return {lhs[1] * rhs[1] - lhs[2] * rhs[2] - lhs[3] * rhs[3] - lhs[4] * rhs[4],  lhs[1] * rhs[2] + lhs[2] * rhs[1] + lhs[3] * rhs[4] - lhs[4] * rhs[3],  lhs[1] * rhs[3] + lhs[3] * rhs[1] + lhs[4] * rhs[2] - lhs[2] * rhs[4],  lhs[1] * rhs[4] + lhs[4] * rhs[1] + lhs[2] * rhs[3] - lhs[3] * rhs[2]}
end

local function math_qdiv(lhs, rhs)
    local l = rhs[1] * rhs[1] + rhs[2] * rhs[2] + rhs[3] * rhs[3] + rhs[4] * rhs[4]

    return {(lhs[1] * rhs[1] + lhs[2] * rhs[2] + lhs[3] * rhs[3] + lhs[4] * rhs[4]) / l,  (-lhs[1] * rhs[2] + lhs[2] * rhs[1] - lhs[3] * rhs[4] + lhs[4] * rhs[3]) / l,  (-lhs[1] * rhs[3] + lhs[3] * rhs[1] - lhs[4] * rhs[2] + lhs[2] * rhs[4]) / l,  (-lhs[1] * rhs[4] + lhs[4] * rhs[1] - lhs[2] * rhs[3] + lhs[3] * rhs[2]) / l}
end
...
local function math_rotationvector(tang, cang)
    ...
    ---- Current --\\
    cang = cang * deg2rad
    qr = {math.cos(cang.r),  math.sin(cang.r),  0,  0}
    qp = {math.cos(cang.p),  0,  math.sin(cang.p),  0}
    qy = {math.cos(cang.y),  0,  0,  math.sin(cang.y)}
...
function ENT:math_angaim(tar, cur)
    ...
    return Angle(dif.p > self.AngThrustMul and self.AngThrustMul or dif.p < -self.AngThrustMul and -self.AngThrustMul or dif.p, dif.y > self.AngThrustMul and self.AngThrustMul or dif.y < -self.AngThrustMul and -self.AngThrustMul or dif.y, 0)
end
...
function ENT:TriggerKey(iname, value)
    if iname == "Level" then
        ...
        self.ShouldLevel = not self.ShouldLevel
...
function ENT:Think()
    ...
    if not IsValid(pphys) then return end
    ...
    lvec, lang = LocalToWorld(Vector(0, 0, 0), self.AngThrust * self.AngThrustMul, pos, ang)
...
function ENT:PostEntityPaste(ply, ent, entlist)
    if not self.EntityMods or not self.EntityMods[self:GetClass()] then return end
...
hook.Add("KeyPress", "tk_ship_engine_keypress", function(ply, key)
    ...
    if not IsValid(pod) or not IsValid(pod.Engine) then return end
    if not IsValid(pod.Engine.Pod) or pod.Engine.Pod ~= pod then return end
    KeyTracker[ply] = KeyTracker[ply] or {}
    KeyTracker[ply][key] = pod.Engine
    pod.Engine:TriggerKey(Inputs[key], 1)
end)

hook.Add("KeyRelease", "tk_ship_engine_keyrelease", function(ply, key)
    if not KeyTracker[ply] then return end
    if not IsValid(KeyTracker[ply][key]) then return end
