...
function ENT:Touch(ent)
	...
	local owner1 = self:CPPIGetOwner()
	local owner2 = ent:CPPIGetOwner()
	if owner1 == owner2 || table.HasValue(owner1:CPPIGetFriends(), owner2) then
		self:AddStorage(ent)
	end
end
...
function ENT:DoThink()
	...
	for k,v in pairs(ents.FindByClass("tk_tib_storage")) do
		if self:LocalToWorld(self:OBBCenter()):Distance(v:LocalToWorld(v:OBBCenter())) < self:BoundingRadius() then
			...
			if owner == entowner || table.HasValue(owner:CPPIGetFriends(), entowner) then
				self:AddStorage(v)
			end
		end
	end
