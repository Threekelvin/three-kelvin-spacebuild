...
--/--- Tib Damage ---\\\
hook.Add("Initialize", "TKTib_Damage", function()
    timer.Create("TKTib_Damage", 1, 0, function()
        for _, ply in pairs(player.GetAll()) do
            for k, v in pairs(ents.FindByClass("tk_tiberium_crystal")) do
                if not IsValid(v) then continue end
                local dist = (v:GetPos() - ply:GetPos()):LengthSqr()
