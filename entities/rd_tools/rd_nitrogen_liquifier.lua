
TOOL.Category    = "Compressors"
TOOL.Name        = "Nitrogen Liquifier"

TOOL.Data        = {
    ["models/ce_ls3additional/compressor/compressor.mdl"]        = {skin = 4, liquid_nitrogen = 50, power = -10},
    ["models/techbot/nitrogenliquifier/nitrogenliquifier.mdl"]     = {liquid_nitrogen = 100, power = -20},
    ["models/ce_ls3additional/compressor/compressor_large.mdl"]    = {skin = 4, liquid_nitrogen = 150, power = -30},
    ["models/ce_ls3additional/compressor/compressor_huge.mdl"]    = {skin = 4, liquid_nitrogen = 200, power = -40}
}