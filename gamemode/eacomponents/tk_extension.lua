...
local function ValidAction(context, ent)
	...
	if !API.Util.IsOwner(context.Player, ent) then return false end
...
local function CreateRDent(context, class, model, pos, angles, freeze)
	...
	if !TK.RD.EntityData[class][model] then
		model = TK.RD.EntityData[class][1]
	end
...
if %TKValidAction( %context, value %1 ) && %TKValidAction( %context, value %2 ) && value %1.IsTKRD && value %2.IsNode then
	%util = value %1:Link( value %2.netid )
end
]], "(%util && true || false)")
Component:AddFunction("unlink", "e:", "b", [[
if %TKValidAction( %context, value %1 ) && value %1.IsTKRD then
	%util = value %1:Unlink( )
end
]], "(%util && true || false)")
...
if %TKValidAction( %context, value %1 ) then
	%util = value %1:GetSequence( ) || 0
end
]], "(%util || 0)")
Component:AddFunction("sequenceGetName", "e:n", "s", [[
if %TKValidAction( %context, value %1 ) then
	%util = value %1:GetSequenceName( value %2 ) || 0
end
]], "(%util || \"\")")
...
local function ValidParticle(particle)
	if table.HasValue(ParticleBlackList, particle) then return false end
...
if %TKValidAction( %context, %ent ) && %ValidParticle( %particle ) then
...
if !%TKValidAction( %context, %this ) && %TKValidAction( %context, %ent ) && %ValidParticle( %particle ) then
...
if $IsValid( value %1 ) && value %1:IsPlayer( ) then
	%util = value %1:IsVip( )
end
]], "(%util && true || false)")
