...
function Build.OnPickUp(ply, ent)
    if not Build.enable:GetBool() then return end
    if not Build.partdata[ent:GetModel()] then return end
    Build.prop = ent
end
...
function Build.HUDPaint()
    if not Build.enable:GetBool() then return end
    if not IsValid(Build.prop) then return end
    ...
    if Build.sprites:GetBool() then
        ...
        for _, tar in pairs(Build.targets) do
            if IsValid(tar) then
                for k, v in pairs(Build.partdata[tar:GetModel()]) do
                    local mat = Build.sprite[v.type][1]
                    local size = Build.sprite[v.type][2]
                    render.SetMaterial(mat)
                    render.DrawSprite(tar:LocalToWorld(v.pos), size[1], size[2], Color(255, 255, 255, 255))
                end
            end
        end
    end
...
hook.Add("PopulateToolMenu", "TK_SBEPBuild", function()
    spawnmenu.AddToolMenuOption("Options", "Player", "SBEPBuildMode", "SBEP Build Mode", "", "", Build.MakeMenu, {
        SwitchConVar = "3k_sbep_build_mode_enabled"
    })
end)
