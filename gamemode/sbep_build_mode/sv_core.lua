...
function Build.CanPickUp(ply, ent)
    ...
    if not Build.partdata[ent:GetModel()] then return false end
...
function Build.CanAttach(ent, ply, eir)
    ...
    if not eir:CPPICanTool(ply, "none") then return false end
...
function Build.CheckPoints(ent, entdata, eir)
    ...
    if not eirdata then return end
    ...
    for _, ap1 in pairs(entdata) do
        for _, ap2 in pairs(eirdata) do
            if Build.Match(ap1, ap2) then
                ...
                if not dist or length < dist then
                    dist = length
    ...
    if dist and dist < 5625 then return point1, point2 end
end
...
function Build.OnPickUp(ply, ent)
    ...
    if not Build.CanPickUp(ply, ent) then return end
...
function Build.KeyRelease(ply, ent, key)
    ...
    if not table.HasValue(Build.elevators, data[5]) then return end
