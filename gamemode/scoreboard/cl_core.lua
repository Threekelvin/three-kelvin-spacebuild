...
-- && then from that point on it pretty much looks after itself. It updates player info
-- in the think function, && removes itself when the player leaves the server.
--
local PLAYER_LINE = {
    Init = function(self)
...
-- of a header && a scrollpanel - into which the player lines are placed.
--
local SCORE_BOARD = {
    Init = function(self)
    ...
    SortPlayers = function(self, force)
        if (CurTime() - self.lastSort < 1 and not force) then return end
        ...
        table.sort(Players, function(a, b)
            ...
            if (a:Team() == b:Team()) then
                ...
                if (a:GetNWInt("TKScore", 0) == b:GetNWInt("TKScore", 0)) then
                    ...
                    if (a:GetNWInt("TKPlaytime", 0) == b:GetNWInt("TKPlaytime", 0)) then return a:Name() < b:Name() end
                end
            end
    ...
    Think = function(self, w, h)
        ...
        --
        -- Loop through each player, && if one doesn't have a score entry - create it.
        --
