...
function PANEL:Init()
    local wide, tall = math.max(ScrW() / 3, 320), math.max(ScrH() / 4, 240)
    self:SetSize(wide, tall)
    self:SetPos(5, ScrH() / 2)
    self.emotelist = {}
    self.inputlog = {}
    self.inputidx = 0
    self.wide = wide
...
function PANEL:PerformLayout()
    ...
    if self.isTeam then
        ...
        local x = surface.GetTextSize("Team")
...
function PANEL:Think()
    ...
    if self.Dragging then
        local x = math.Clamp(gui.MouseX() - self.Dragging[1], 0, ScrW() - wide)
        local y = math.Clamp(gui.MouseY() - self.Dragging[2], 0, ScrH() - tall)
        self:SetPos(x, y)
    end

    if self.Sizing then
        local px, py = self:GetPos()
        local x = math.Clamp(gui.MouseX() - self.Sizing[1], self.isOpen and 320 + 145 or 320, ScrW() - px)
        local y = math.Clamp(gui.MouseY() - self.Sizing[2], 240, ScrH() - py)
        self:SetSize(x, y)
    ...
    if self.Hovered and not self.moving and gui.MouseY() <= self.y + 5 then
        self:SetCursor("sizeall")

        return
    elseif self.Hovered and not self.moving and gui.MouseX() >= self.x + wide - 5 and gui.MouseY() >= self.y + tall - 5 then
...
function PANEL:OnMousePressed()
    if not self.moving and gui.MouseX() >= self.x + self:GetWide() - 5 and gui.MouseY() >= self.y + self:GetTall() - 5 then
    ...
    if not self.moving and gui.MouseY() <= self.y + 5 then
