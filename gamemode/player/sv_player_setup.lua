...
function TK:SetSpawnPoint(ply, side)
    ...
    if not Spawn then
        Spawn = self.MapSetup.SpawnPoints[1]
    end
...
function GM:PlayerInitialSpawn(ply)
    ...
    http.Fetch(string.format("http://resource.threekelv.in/avatar.php?steamid=%s", ply:SteamID()), function() end, function() end)
end
...
hook.Add("PlayerLeaveVehicle", "3k_Vehicle_Exit", function(ply, ent)
    ...
    for k, v in pairs(ent:GetConstrainedEntities()) do
        if not v.VehicleExitPoint then continue end
        hasExit = v
        break
    end
