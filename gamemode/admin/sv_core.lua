        ...
        if v == "*" then
            ...
            for l, b in pairs(player.GetAll()) do
                if TK.AM:Match(b, v) then
                    if ply:CanRunOn(b) and not table.HasValue(Targets, b) then
...
util.AddNetworkString("TKAFK")

function TK.AM:SetAFK(ply, isAFK, txt)
    if not IsValid(ply) then return end

    if ply:IsAFK() and not isAFK then
        ply:SetNWBool("TKAFK", false)

        timer.Create("AFK_" .. ply:UserID(), 600, 1, function()
            TK.AM:SetAFK(ply, true)
        end)
    ...
    elseif not ply:IsAFK() and isAFK then
...
hook.Add("KeyPress", "TKAFKTimer", function(ply, key)
    if not table.HasValue(Inputs, key) then return end
...
--/--- ---\\\
--/--- Functions ---\\\
util.AddNetworkString("TKStopSounds")

function TK.AM:StopSounds(ply)
    if not IsValid(ply) then return end
    net.Start("TKStopSounds")
    net.Send(ply)
end
