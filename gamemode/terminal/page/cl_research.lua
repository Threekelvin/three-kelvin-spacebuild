...
local function MakeResearchBox(panel, idx, data)
    ...
    btn.Paint = function(self, w, h)
        derma.SkinHook("Paint", "TKUpPanel", self, w, h)

        return true
    end

    btn.DoClick = function()
        if not IsValid(panel.Terminal) then return end
        ...
        frame.Paint = function(self, w, h)
            derma.SkinHook("Paint", "TKUpFrame", self, w, h)
        ...
        cost.Paint = function(self, w, h)
            panel:SetFontInternal("TKFont15")
            derma.SkinHook("Paint", "TKTopTextBox", self, w, h)
