...
function PP:BlackList(ent, mdl, class)
    ...
    elseif mdl then
        if table.HasValue(TK.PP.PropBlackList, mdl) then return true end
    elseif class then
        if table.HasValue(TK.PP.EntityBlackList, class) then return true end
    end
...
function PP:UpdateBuddy(ply, tid, flag)
    ...
    if self:HasPermission(flag, "CPPI") ~= cppi then
        ...
        for k, v in pairs(self.BuddyTable[uid]) do
            if not self:HasPermission(v, "CPPI") then continue end
...
function PP:CanOverride(ply, typ, dir, tar)
    if IsValid(tar) and tar:IsPlayer() then
        if not ply:CanRunOn(tar) then return false end
    end
...
function PP:IsBuddy(uid, ply, typ)
    ...
    if not self.BuddyTable[uid] then return false end
...
concommand.Add("pp_cleanup", function(ply, cmd, arg)
    if ply:IsModerator() then
        ...
        elseif uid == ply:UID() then
            ...
            if not ply:CanRunOn(tar) then return end
...
function PP:CanToolEnt(ply, toolmode, ent)
    ...
    if IsValid(owner) then
        if string.match(toolmode, "duplicator") or string.match(toolmode, "dupe") then
            if not self:CheckConstraints(ply, ent, "Dupe") then return false end
        ...
        elseif toolmode == "remover" and (ply:KeyDown(IN_ATTACK2) or ply:KeyDownLast(IN_ATTACK2)) then
            if not self:CheckConstraints(ply, ent, "Tool Gun") then return false end
        end
...
function PP:CanToolGun(ply, tr, toolmode)
    if IsValid(ply.InShip) then return false end
    if not tr.HitNonWorld or not IsValid(tr.Entity) then return end
    local ent = tr.Entity
    if ent:IsPlayer() then return false end
    local owner, uid = self:GetOwner(ent)
...
function PP:CanPhysGunReload(wep, ply)
    ...
    if tr.HitNonWorld and IsValid(tr.Entity) then
        ...
        if IsValid(owner) then
            if not self:CheckConstraints(ply, tr.Entity, "Phys Gun") then return false end
            if ply == owner then return end
            if self:CanOverride(ply, "Phys Gun", "Prop", owner) then return end
            if self:IsBuddy(uid, ply, "Phys Gun") then return end
...
hook.Add("PlayerSpawnSENT", "TKPP", function(ply, class)
    if PP:BlackList(nil, nil, class) then return false end
end)

hook.Add("PlayerSpawnProp", "TKPP", function(ply, mdl)
    if PP:BlackList(nil, mdl) then return false end
end)
...
hook.Add("PlayerSpawnedRagdoll", "TKPP", function(ply, mdl, ent)
    PP:SetOwner(ply, ent)
end)

hook.Add("PlayerSpawnedProp", "TKPP", function(ply, mdl, ent)
    PP:SetOwner(ply, ent)
end)

hook.Add("PlayerSpawnedEffect", "TKPP", function(ply, mdl, ent)
    PP:SetOwner(ply, ent)
end)

hook.Add("PlayerSpawnedVehicle", "TKPP", function(ply, ent)
    PP:SetOwner(ply, ent)
end)

hook.Add("PlayerSpawnedNPC", "TKPP", function(ply, ent)
    PP:SetOwner(ply, ent)
end)

hook.Add("PlayerSpawnedSENT", "TKPP", function(ply, ent)
    PP:SetOwner(ply, ent)
end)
...
util.AddNetworkString("PP_Menu1")
util.AddNetworkString("PP_Menu2")
